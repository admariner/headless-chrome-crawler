...
class HCCrawler extends EventEmitter {
  ...
  queue(options) {
    each(isArray(options) ? options : [options], _options => {
      this._push(this._buildOptions(_options), 1);
    });
  }

  _buildOptions(_options) {
    const requestOptions = isString(_options) ? { url: _options } : _options;
    each(CONSTRUCTOR_OPTIONS, option => {
      if (requestOptions[option]) throw new Error(`Overriding ${option} is not allowed!`);
    });
    const mergedOptions = extend({}, this._options, requestOptions);
    ...
    return omit(mergedOptions, CONSTRUCTOR_OPTIONS);
  }

  request(options) {
    return this._startSyncRequest(this._buildOptions(options), 1);
  }

  // FIXME: try to factorize with _request
  _startSyncRequest(options, depth, retryCount = 0) {
    ...
    return this._newPage(options)
      .then(crawler => {
        ...
        return crawler.crawl()
          .then(res => {
            res = extend({ options, depth: 1 }, res);
          ...
          .catch(error => {
            ...
            return crawler.close()
              ...
              .then(() => this._startSyncRequest(options, depth, retryCount + 1));
          })
          .catch(error => {
      ...
      .then(skip => {
        ...
        return this._markRequested(options)
          ...
          .then(({ redirect, links }) => {
            ...
            if (redirect) {
              return this._followUrls([redirect], options, depth);
            }
            return this._followUrls(links, options, depth + 1);
          })
  ...
  _skipRequest(options) {
    return Promise.resolve(this._checkAllowedDomains(options))
      .then(allowedDomain => {
        if (!allowedDomain) return true;
        return this._checkRequested(options)
          .then(requested => {
            if (requested) return true;
            return this._shouldRequest(options)
              .then(shouldRequest => {
                if (!shouldRequest) return true;
                return this._checkAllowedRobots(options)
                  .then(allowedRobot => !allowedRobot);
              });
          });
      });
  }
        ...
        return crawler.crawl()
          .then(res => {
            ...
            return this._success(res)
              ...
              .then(() => crawler.close())
              .then(() => ({
                redirect: resolveUrl(res.response.headers.location, options.url),
                links: res.links,
              }));
          })
          .catch(error => {
   ...
   * @private
   */
  _followUrls(urls, options, depth) {
