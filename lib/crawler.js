...
class Crawler {
  ...
  _setFollowRedirects() {
    if (this._options.followRedirects) return Promise.resolve();
    // FIXME should be mutualised with other potential features (ex: skip images, etc.)
    return this._page.setRequestInterception(true)
      .then(() => this._page.on('request', request => {
        // FIXME should be done different to mutualize the request interception logic
        if (request.resourceType() === 'document' && request.url() !== this._options.url) {
          // if this is a request of type 'document' with a different target url then it's a redirection.
          // Let's resolve it with an empty body to not follow it.
